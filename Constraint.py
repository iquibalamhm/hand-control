...
class Constraint:
    ...
    def StopConstraint(self):
        self.target = math.sqrt((self.node2.position.x - self.node1.position.x)**2 + (self.node2.position.y - self.node1.position.y)**2)
        self.stiff = 1.0
        #D = self.node2.position - self.node1.position
        #F = Vector.zero()
        self.node1.StopParticleMotion()
        self.node2.StopParticleMotion()