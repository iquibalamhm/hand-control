...
class Particle:
    ...
    def Simulate(self):
        ...
        self.position = self.velocity*(1.0-damping) + self.acceleration * self.world.delta**2.0
        self.velocity = self.position - self.previous
        self.acceleration = Vector.zero()

    def StopParticleMotion(self):
        if not self.material.mass:
            return
        self.previous = self.position
        self.position = self.position
        #self.velocity + self.acceleration * self.world.delta**2.0
    ...
    def ApplyForce(self, force):
        if self.material.mass != 0.0:
            force.x = force.x / self.material.mass
            force.y = force.y / self.material.mass
    ...
    def ApplyImpulse(self, impulse):
        if self.material.mass != 0.0:
            impulse.x = impulse.x /self.material.mass
            impulse.y = impulse.y / self.material.mass
